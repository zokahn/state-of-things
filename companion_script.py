    ...
    if response.status_code == 200:
        ...
        print()
            ...
            for entity in entities:
                response =
                requests.get(api_url + '/' + entity + '/?project_id = ' + project_id, headers = headers)
                if response.status_code == 200:
                    items = response.json()
                    ...
                    for item in items:
                        ...
                        print()
                            ...
                            write_mode(api_url, api_token, project_id):
                                headers = {'Authorization': 'Bearer ' + api_token}
                                entity_type =
                                input('Enter the type of item to add (task, issue, design_rule, requirement, project_goal, sbom): ').lower()
